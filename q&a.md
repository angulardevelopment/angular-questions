...
Explain Signals, modern reactivity, and local change detection
What is the difference between linkedSignal and computed signal?
Why does the Resource API not trigger interceptors?
You are using httpResource to fetch data in a service. In the component, will you read the value using computed or effect, and why?
Give two examples where you would use an effect instead of a computed signal
You have a computed signal, but due to some business logic, you need to set its value manually. What will you do
Effects created in a service vs effects created inside components—how are they different?
